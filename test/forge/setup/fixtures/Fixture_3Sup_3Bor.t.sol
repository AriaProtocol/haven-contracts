...
contract Fixture_3Sup_3Bor is Common_Setup {
    /// @dev so we dont have to pass it through all functions
    CometMainInterface internal _comet;
    ...
    function _loadFixture(CometMainInterface comet_) internal {
        ...
        baseToken.allocateTo(address(_comet), RESERVES); // Fund reserves
    ...
    function _setupSuppliers() internal {
        ...
        baseToken.approve(address(_comet), 1_000_000e6);
        _comet.supply(address(baseToken), 1_000_000e6);
        vm.stopPrank();
    ...
    function _setupBorrowers() internal {
        ...
        wbtc.approve(address(_comet), 15e8);
        _comet.supply(address(wbtc), 15e8);
        _comet.withdraw(address(baseToken), 800_000e6);
        vm.stopPrank();
        ...
        weth.approve(address(_comet), 350e18);
        _comet.supply(address(weth), 350e18);
        _comet.withdraw(address(baseToken), 800_000e6);
        vm.stopPrank();
        ...
        wbtc.approve(address(_comet), 10e8);
        weth.approve(address(_comet), 100e18);
        _comet.supply(address(wbtc), 10e8);
        _comet.supply(address(weth), 100e18);
        _comet.withdraw(address(baseToken), 800_000e6);
        vm.stopPrank();
    }
    ...
    function _updateBorrowInvariant() internal {
        invariant.totalBorrow = _comet.totalBorrow();
        invariant.reserves = _comet.getReserves();
        invariant.utilization = _comet.getUtilization();

        ___updateCollateralInvariant(address(weth));
        ___updateCollateralInvariant(address(wbtc));
    }
    ...
    function ___updateCollateralInvariant(address token) private {
        invariant.collaterals[token].collateralReserves = _comet.getCollateralReserves(token);

        (, bytes memory data) = address(_comet).staticcall(abi.encodeWithSignature("totalsCollateral(address)", token));
