...
contract GovernorSimple {
    ...
    function initialize(address timelock_, address[] memory admins_) external {
        require(
            ...
            "GovernorSimple::initialize: can only initialize once"
        );
    ...
    function propose(
        ...
        require(
            isAdmin(msg.sender),
            "GovernorSimple::propose: only governors can propose"
        );
        require(
        ...
        require(
            targets.length != 0,
            "GovernorSimple::propose: must provide actions"
        );
        require(
            targets.length <= proposalMaxOperations,
            "GovernorSimple::propose: too many actions"
        );
    ...
    function queue(uint proposalId) external {
        require(
            isAdmin(msg.sender),
            "GovernorSimple::queue: only governors can queue"
        );
        require(
            state(proposalId) == ProposalState.Active,
            "GovernorSimple::queue: proposal can only be queued if it is active"
        );
    ...
    function queueOrRevertInternal(
        ...
        require(
            !timelock.queuedTransactions(
                keccak256(abi.encode(target, value, signature, data))
            ),
    ...
    function execute(uint proposalId) external payable {
        require(
            isAdmin(msg.sender),
            "GovernorSimple::execute: only governors can execute"
        );
        require(
            state(proposalId) == ProposalState.Queued,
            "GovernorSimple::execute: proposal can only be executed if it is queued"
        );
    ...
    function cancel(uint proposalId) external {
        require(
            isAdmin(msg.sender),
            "GovernorSimple::cancel: only governors can cancel"
        );
        require(
            state(proposalId) != ProposalState.Executed,
            "GovernorSimple::cancel: cannot cancel executed proposal"
        );
    ...
    function state(uint proposalId) public view returns (ProposalState) {
        require(
            ...
            "GovernorSimple::state: invalid proposal id"
        );
    ...
    function addAdmin(address newAdminAddress) external {
        require(
            isAdmin(msg.sender),
            "GovernorSimple::addAdmin: only governors can add governors"
        );
    ...
    function removeAdmin(address adminAddress) external {
        require(
            isAdmin(msg.sender),
            "GovernorSimple::removeAdmin: only governors can remove governors"
        );
        require(
            msg.sender != adminAddress,
            "GovernorSimple::removeAdmin: cannot remove self as admin"
